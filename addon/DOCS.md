...
| `frequency` | Tuner frequency (`-f`). Default `433.92M`. |
| `gain` | Tuner gain (`-g`). Default `40`. Blank = rtl_433 default. |
| `ppm` | Crystal correction (`-p`). Default `0`. |
| `classic_demod` | Use `-Y classic` (default `true` in 0.1.19). |
